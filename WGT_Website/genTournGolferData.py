class JSONTournGolfer (object):
    def __init__ (self, tournId, golfer):     
        self.model = "golfer.tourngolfer"
        self.fields = {"tg_tourn": tournId,
                       "tg_golfer": golfer}
...
def getJSONstr():
    ...
    # Empty list to hold new TournGolfer data
    tourn_golfers = []
    
    # Call getTopTenGolfers to look through golfer list
    top_ten_list = getTopTenGolfers()
    
    for golfer in top_ten_list:
        golfer_id = golfer[1]
        tourn_golfer = JSONTournGolfer (tourn_id, golfer_id)
        json_tourn_golfer = json.dumps (tourn_golfer.__dict__)
        tourn_golfers.append (json_tourn_golfer)
